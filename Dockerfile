...
HEALTHCHECK --interval=15s --timeout=15s --start-period=5s --retries=3 \
  CMD celery -A app.celery inspect ping -d $QUEUE@$HOSTNAME

CMD celery -A app.celery worker -Q $QUEUE -c ${CONCURENCY:-2} -n "${QUEUE}@%h" --max-tasks-per-child ${MAX_REQUESTS_PER_CHILD:-50} --time-limit ${TIME_LIMIT:-900} --soft-time-limit ${SOFT_TIME_LIMIT:-45}
...
HEALTHCHECK NONE
CMD celery -A app.celery beat --pidfile=/tmp/celerybeat.pid --schedule=${_SCHEDULER_DB_PATH}/celerybeat-schedule.db
